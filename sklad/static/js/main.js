...
function changeUploader(number){
    ...
    var html_code = '<div class="col-4 divNameT">\n' +
        '<label for="uploader_car'+ indTran +'" class="nameTable">Транспорт для розвантження '+indTran+'</label></div>\n' +
...
function StopAddItem() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data["count"] == 0){
                var html_code = '<option value="0">Немає доступних до розміщення</option>';
                $("#traffic5").append(html_code);
            }
        }
    });
...
function addNewItem() {
    ...
    var html_code = '<div class="col-3 h36 pad15TB">\n' +
        ...
        '                            </div>\n' +
        '                            <div class="col-3">\n' +
        '                                <div class="center"><input id="'+ id3 +'" type="date" name="'+ id3 +'" placeholder="Термін придатності до"></div>\n' +
        '                            </div>\n' +
        '                            <div class="col-3">\n' +
        '                                <div class="center"><input id="'+ id4 +'" type="number" name="'+ id4 + '" placeholder="кількість палет"></div>\n' +
        '                            </div>';
...
$('#car_type_Boolean').change(function() {
    if ($('#car_type_Boolean').val() === "Розвантаження/Завантажений"){
        $("#palletTruck").hide();
    }
    else {
        $("#palletTruck").show();

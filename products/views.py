...
def show_index(request):
    ...
    prd_sx = Product.objects.filter(status="Відправлен").count()
    prd_st = Product.objects.filter(status="У обробці").count()
    prd_ht = Product.objects.filter(status="Прийнятий").count()
    prd_ot = Product.objects.filter(status="Скасований").count()

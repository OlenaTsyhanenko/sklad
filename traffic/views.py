...
def stopTraffic2(request, id1, id3):
    ...
    if status == "cancell":
        unl.status = "Скасований"
    else:
        unl.status = "Прийнятий"
    unl.unloads = False
...
def stopTraffic(request, id1, id3):
    ...
    if status == "cancell":
        trf.status = "Скасований"
    else:
        trf.status = "Прийнятий"
    trf.unloads = False
...
def startUnload(request, id1):
    ...
    tr.status = "Завантажений"
    tr.loads = True
    tr.save()
    pr = Product.objects.filter(Unloader=tr)
    for ch in pr:
        ch.status = "Завантажений"
        ch.save()
    data = datetime.now()
...
def addNewEnd(request):
    ...
    tr = Traffic.objects.create(name=name1, dateRegister=datetime.now(), sklad=sk, status="Відправлен",
                                dateArrival=date1)
    while i < count:
        ...
        pr = Product.objects.create(name=nameProd, optimumTemperature=tempure, shelfLife=life,
                                    ...
                                    status="Відправлен", height=height)
        pr.save()
        i += 1
...
def addNew(request):
    ...
    for ch in sklad:
        ...
        pr1 = Product.objects.filter(sklad=ch, status="Відправлен")
        pr2 = Product.objects.filter(sklad=ch, status="Прибув")
        pr3 = Product.objects.filter(sklad=ch, status="Прийнятий")
        batchall = 0
        for tov in pr1:
...
def unloading(request):
    ...
    if auth.get_user(request).username.__len__() > 0:
        ...
        profile = Profile.objects.get(user=auth.get_user(request))
        type_user = profile.position
    prd = Product.objects.filter(status="Прийнятий")
...
def show_index(request):
    ...
    tr1 = Traffic.objects.filter(status="Відправлен")
    tr2 = Traffic.objects.filter(status="Прибув")
    tr3 = Traffic.objects.filter(status="Прийнятий")
    tr4 = Traffic.objects.filter(status="Скасований")

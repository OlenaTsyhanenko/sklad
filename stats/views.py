...
def getSt(request):
    ...
    if sk == "9999999":
        trafficNow = Traffic.objects.filter(status="Прийнятий", dateUpload__month=dateL.month)
        trafficWait = Traffic.objects.filter(status="Відправлен", dateArrival__month=dateL.month)
    else:
        trafficNow = Traffic.objects.filter(status="Прийнятий", dateUpload__month=dateL.month, sklad=WarehouseSpace.objects.get(id=sk))
        trafficWait = Traffic.objects.filter(status="Відправлен", dateArrival__month=dateL.month, sklad=WarehouseSpace.objects.get(id=sk))

    unload_now = Unloader.objects.filter(status="Відправлен", dateUpload__month=dateL.month)
    trW1 = 0
    trW2 = 0
    trW3 = 0
    trW4 = 0
